...
// Honour `--port <n>` / `--port=<n>` from the CLI (the Lovable dev daemon
// starts this with `--port 8080`); fall back to PORT env, then 5000.
const portArgIndex = process.argv.indexOf("--port");
const portFromFlag =
  portArgIndex !== -1 ? process.argv[portArgIndex + 1] : process.argv.find((a) => a.startsWith("--port="))?.split("=")[1];
const PORT = Number(portFromFlag) || Number(process.env.PORT) || 5000;
