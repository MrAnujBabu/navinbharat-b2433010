...
import { safeFetch, SsrfBlockedError, isBlockedHost } from "../_shared/ssrfGuard.ts";
...
Deno.serve(async (req) => {
  ...
  // SSRF guard: block private/loopback/link-local hosts (literal + DNS).
  if (await isBlockedHost(target.hostname)) {
    return json({ error: "URL host not allowed" }, 400);
  }

  // Fetch remote image. Redirects are followed manually and every hop is
  // re-validated, so a remote 302 cannot pull us into an internal address.
  let remote: Response;
  try {
    remote = await safeFetch(target.toString(), {
