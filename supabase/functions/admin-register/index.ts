...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Brute-force guard: the admin code is the ONLY thing standing between an
    // anonymous caller and an admin account, so cap attempts per client IP
    // (5 / hour) before doing any comparison or account work.
    const clientIp =
      (req.headers.get('x-forwarded-for') || '').split(',')[0].trim() ||
      req.headers.get('cf-connecting-ip') ||
      'unknown'
    const { data: rlAllowed, error: rlError } = await supabaseAdmin.rpc('check_rate_limit_text', {
      _bucket: 'admin_register',
      _identifier: clientIp,
      _max: 5,
      _window_seconds: 3600,
    })
    if (rlError) return json({ success: false, error: 'Rate limit check failed' }, 503)
    if (rlAllowed === false) {
      await supabaseAdmin.from('security_alerts').insert({
        alert_type: 'admin_register.rate_limited',
        details: { email },
        source_ip: clientIp,
      })
