...
export async function saveAndIndexDownload(
  ...
  const saveBlobToIndexedDb = async (blob: Blob): Promise<SaveDownloadResult> => {
    ...
    try {
      ...
      // Crash-shield: never leave a `web-indexeddb:pending` index row behind
      // when the blob write fails (quota / storage eviction). An orphan row
      // renders as a broken "saved" item that throws on every open attempt.
      try { await downloadFileDB.delete(id); } catch { /* best effort */ }
      try { await dbDelete(id); } catch { /* best effort */ }
      throw err;
    }
