...
import { DRIVE_DOWNLOAD_DISABLED_MSG } from "./driveBlockDiagnosis";

export { DRIVE_DOWNLOAD_DISABLED_MSG };
...
export function friendlyPdfErrorMessage(err: unknown, src: string | null | undefined): string {
  ...
  // Shared publicly, but the owner disabled download/print/copy — a retry can
  // never succeed, so never show the "enable link sharing" copy here.
  if (/drive_download_disabled|disabled downloading for this Drive file/i.test(`${code} ${msg}`)) {
    return DRIVE_DOWNLOAD_DISABLED_MSG;
  }
