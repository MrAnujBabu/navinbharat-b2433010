...
/**
 * A rejected non-Error (`reject({})`, a plugin result object, a string) used to
 * reach Sentry as `[crashShield] unhandledrejection {}` — an issue nobody can
 * ever act on. Flatten whatever we got into something readable, and if there is
 * genuinely nothing in it, count it but don't report it.
 */
function describeReason(reason: unknown): string {
  ...
  const parts: string[] = [];
  for (const key of ["name", "code", "status", "message", "error", "errorMessage"]) {
    if (r[key] != null && r[key] !== "") parts.push(`${key}=${String(r[key])}`);
  }
  if (parts.length === 0) {
...
function installGlobalTraps() {
  window.addEventListener("unhandledrejection", (e) => {
    ...
    rejectionCount += 1;
    const described = describeReason(e.reason);
    if (described) {
      console.error(`[crashShield] unhandledrejection ${described}`, e.reason);
    }
    try { e.preventDefault(); } catch { /* noop */ }
