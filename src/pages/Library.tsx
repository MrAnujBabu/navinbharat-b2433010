...
import { WifiOff, HardDrive, Plus, Loader2, FolderOpen } from "lucide-react";
            ...
            <TabsContent key={l.key} value={l.key} className="mt-3 space-y-4">
              {loading ? (
                ...
                <div className="rounded-2xl border border-border bg-card p-6 text-center">
                  <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/20 to-accent/20 text-primary">
                    {online ? <FolderOpen className="h-6 w-6" /> : <WifiOff className="h-6 w-6" />}
                  </div>
                  <p className="text-sm font-semibold text-foreground">
                    {online ? `No ${l.label.toLowerCase()} PDFs yet` : "You're offline"}
                  </p>
                  <p className="mx-auto mt-1 max-w-[38ch] text-xs text-muted-foreground">
                    {online
                      ? "Add a PDF from this device, or open a lesson and download its notes to keep them here."
                      : "Download PDFs while online to access them anytime, even without a network."}
                  </p>
                  {online && (
