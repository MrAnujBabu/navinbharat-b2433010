...
async function reconcile(force = false) {
  ...
  try {
    ...
    // Leave `enabled` as-is so the next reconcile retries. Never alert the
    // student, but leave a redacted diagnostic trail: a silent FLAG_SECURE
    // failure otherwise looks identical to working protection.
    try {
      const { logger } = await import("@/lib/logger");
      logger.warn("screen-protection: native reconcile failed", {
        desired,
        force,
