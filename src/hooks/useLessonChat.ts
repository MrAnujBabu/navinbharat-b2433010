...
export function useLessonChat(
  ...
  const invokeAI = useCallback(
    async (message: string, history: { role: string; content: string }[]) => {
      ...
      // Retry transient function/upstream failures ONLY. A successful call must
      // never be re-sent (that tripled latency, credits and rate-limit usage).
      // Auth / billing / validation errors are terminal.
      for (let attempt = 0; attempt < 2; attempt++) {
        const apiCode = (data as { code?: string } | null)?.code;
        const hasFailure = !!fnErr || !!(data as { error?: string } | null)?.error;
        if (!hasFailure) break;
        const status = (fnErr as { context?: { status?: number } })?.context?.status;
        const transient =
          apiCode !== "gateway_unauthorized" &&
          apiCode !== "credits_exhausted" &&
  ...
  const sendChat = useCallback(async (overrideText?: string) => {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : "AI could not answer right now";
      toast.error(msg);
