...
describe("ReaderProgress staged PDF readiness", () => {
  ...
  it("ignores progress and readiness from another mounted reader", () => {
    render(<ReaderProgress visible title="Current PDF" variant="pdf" readerId="current" />);

    act(() => progress({ readerId: "background", percent: 80, phase: "downloading" }));
    expect(screen.getByRole("progressbar")).not.toHaveAttribute("aria-valuenow", "80");

    act(() => progress({ readerId: "current", percent: 30, phase: "downloading" }));
    expect(screen.getByRole("progressbar")).toHaveAttribute("aria-valuenow", "30");

    act(() => window.dispatchEvent(new CustomEvent("pdf-ready", { detail: { readerId: "background" } })));
    expect(screen.getByRole("progressbar")).toHaveAttribute("aria-valuenow", "30");
  });
});