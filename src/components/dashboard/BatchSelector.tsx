...
import { useResolvedContentUrlString } from "../../hooks/useResolvedContentUrl";
import coursePlaceholder from "../../assets/thumbnails/course-default.svg";

/**
 * Batch thumbnails live in the private `content` bucket, so the raw
 * `/object/public/content/...` URL stored on the course 404s. Resolve it to a
 * signed URL and fall back to the shared course placeholder instead of
 * rendering an empty square.
 */
const BatchThumb = ({ src, size }: { src?: string | null; size: "sm" | "md" }) => {
  const resolved = useResolvedContentUrlString(src);
  if (!src) return null;
  return (
    <img
      src={resolved ?? coursePlaceholder}
      alt=""
      loading="lazy"
      decoding="async"
      onError={(e) => {
        (e.currentTarget as HTMLImageElement).src = coursePlaceholder;
      }}
      className={
        size === "md"
          ? "h-7 w-7 rounded-lg object-cover flex-shrink-0 bg-muted"
          : "h-6 w-6 rounded-md object-cover flex-shrink-0 bg-muted"
      }
    />
  );
};
      ...
      >
        <SelectTrigger className="w-full min-w-0 bg-card border border-border rounded-xl h-12 pl-4 pr-3 text-sm sm:text-base font-semibold text-foreground shadow-sm">
          <div className="flex flex-1 min-w-0 items-center gap-2.5 overflow-hidden text-left">
            <BatchThumb src={selectedBatch?.image_url} size="md" />
            ...
            return (
              <SelectItem key={batch.id} value={batch.id.toString()}>
                <div className="flex min-w-0 items-center gap-2.5">
                  <BatchThumb src={batch.image_url} size="sm" />
