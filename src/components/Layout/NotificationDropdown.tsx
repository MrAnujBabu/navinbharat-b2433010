...
const NotificationDropdown = () => {
  ...
  /* Cached per user: the bell mounts on every authed page, so an uncached
     fetch meant two round-trips (notices + reads) on each navigation. */
  const { data: notices = [] } = useQuery({
    queryKey: ["notification-bell", user?.id ?? "anon"],
    staleTime: 60_000,
    gcTime: 5 * 60_000,
    ...
    queryFn: async (): Promise<Notice[]> => {
      ...
      if (user) {
        ...
        readIds = new Set((reads || []).map((r: { notice_id: string }) => r.notice_id));
      }
      return noticeData.map((n) => ({ ...n, isRead: readIds.has(n.id) }));
    },
  });
