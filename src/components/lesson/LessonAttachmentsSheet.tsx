...
export function LessonAttachmentsSheet({ open, onOpenChange, lessonId, lessonTitle, lessonType }: Props) {
  ...
  const handleOpenPdf = useCallback(async (url: string, fileName: string, note?: LessonNote) => {
    ...
    // Storage ids must never reach the reader overlay — fall back to the note
    // title, then the lesson title.
    const decodedName = pdfDisplayName(safeDecodeFileName(fileName) || fileName, [
      note?.title,
      lessonTitle,
    ]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!isMountedRef.current || seq !== requestSeqRef.current) return;
      const decodedName = pdfDisplayName(safeDecodeFileName(only.file_name) || only.file_name, [
        only.title,
        lessonTitle,
      ]);
