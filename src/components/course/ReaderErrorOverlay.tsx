...
const ReaderErrorOverlay = memo(({ message, onRetry, onOpenExternal }: Props) => (
  <div
    role="alert"
    // z-20: MUST stay below the reader header (z-40). At z-30 this overlay
    // shared the header's stacking level and, being later in DOM order,
    // painted on top of it — which silently killed every header control
    // (back / bookmark / download / fullscreen) whenever a load failed.
    // The top padding keeps the message clear of the header itself.
    className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-4 bg-background/95 px-6 pt-20 text-center"
  >
