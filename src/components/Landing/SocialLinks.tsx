      ...
      {links.map((link) => {
        ...
        return (
          <a
            ...
            onClick={(e) => {
              // Inside the Capacitor WebView a bare target="_blank" is a no-op.
              if (isNativeSync()) {
                e.preventDefault();
                void openSocialLink(link.value);
              }
            }}
            className="inline-flex min-h-11 items-center gap-1.5 px-3 py-1.5 rounded-full bg-muted/60 hover:bg-primary/10 text-sm text-muted-foreground hover:text-primary transition-colors"
            title={info.label}
          >
            <span>{info.icon}</span>
